...
class ConstructionDashboard(models.Model):
    ...
    def get_construction_state(self, site_id, project_id):
        ...
        if (not site_id and not project_id) or (site_id == 'all_site' and project_id == 'all_project'):
            ...
            projects = self.env['tk.construction.project'].search(site_domain).mapped('id')
        ...
        total_project = self.env['tk.construction.project'].search_count(site_domain)
        total_mrq = self.env['material.requisition'].search_count(project_domain)
    ...
    def get_mrq_state(self, project_domain):
        ...
        draft = material_requisition.search_count([('stage', '=', 'draft')] + project_domain)
        waiting_approval = material_requisition.search_count([('stage', '=', 'department_approval')] + project_domain)
        approved = material_requisition.search_count([('stage', '=', 'approve')] + project_domain)
        ready_delivery = material_requisition.search_count([('stage', '=', 'ready_delivery')] + project_domain)
        material_arrive = material_requisition.search_count([('stage', '=', 'material_arrived')] + project_domain)
        internal_transfer = material_requisition.search_count([('stage', '=', 'internal_transfer')] + project_domain)
        reject = material_requisition.search_count([('stage', '=', 'reject')] + project_domain)
        cancel = material_requisition.search_count([('stage', '=', 'cancel')] + project_domain)
        return [
            ['Draft', 'Waiting Approval', 'In Progress', 'Ready Delivery', 'Material Arrive', 'Internal Transfer',
             'Reject', 'Cancel'],
            [draft, waiting_approval, approved, ready_delivery, material_arrive, internal_transfer, reject, cancel]
        ]
    ...
    def get_it_state(self, site_domain):
        ...
        draft = internal_transfer.search_count([('stage', '=', 'draft')] + site_domain)
        in_progress = internal_transfer.search_count([('stage', '=', 'in_progress')] + site_domain)
        done = internal_transfer.search_count([('stage', '=', 'done')] + site_domain)
        return [
    ...
    def get_project_status(self, site_domain):
        project_obj = self.env['tk.construction.project'].sudo()
        planning = project_obj.search_count([('stage', '=', 'Planning')] + site_domain)
        procurement = project_obj.search_count([('stage', '=', 'Procurement')] + site_domain)
        construction = project_obj.search_count([('stage', '=', 'Construction')] + site_domain)
        handover = project_obj.search_count([('stage', '=', 'Handover')] + site_domain)
        return [
    ...
    def get_purchase_order_state(self, project_domain):
        ...
        mr_po = po_obj.search_count([('material_req_id', '!=', False)] + domain)
        equip_po = 0
        labour_po = 0
        overhead_po = 0
